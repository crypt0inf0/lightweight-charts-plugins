# Lightweight Charts Market Depth Tool
...
This package provides a specialized **Market Depth** visualization tool for the [Lightweight Charts v5+](https://github.com/tradingview/lightweight-charts) plugin system. It allows developers to visualize supply and demand (Order Book) levels directly on the price scale, providing a real-time view of market liquidity. It is part of a modular, high-performance **drop-in replacement** suite for the legacy [v3.8 Line Tools Build](https://github.com/difurious/lightweight-charts-line-tools).
...
1.  **Clone the repository**:
    ```bash
    git clone https://github.com/difurious/lightweight-charts-line-tools-market-depth.git
    cd lightweight-charts-line-tools-market-depth
    ```
...
4.  **Output**:
    ...
    *   `/docs`: Technical API documentation. Open `docs/index.html` and navigate to the `LineToolMarketDepth` class to view all available configuration options (bid/ask colors, line offsets, data structures) and methods.
...
Here is how to register and use the Market Depth tool in your chart. 
NOTE, market depth needs bid and ask data, and is not intended to be created via an interactive method.  You would recieve the market depth data from the broker, then on every data update, update the coresonding MarketDepth line tool's ID to update the data on screen.
...
// 2. Register the Market Depth Tool
lineTools.registerLineTool('MarketDepth', LineToolMarketDepth);

// 3. Start interactive drawing mode (Click to place the horizontal anchor)
// Note: This tool typically requires an options object containing the 'marketDepthData' 
// (Bids/Asks) to display meaningful results.
lineTools.addLineTool('MarketDepth');
```
