# Lightweight Charts Price Range Tool
...
This package provides a specialized **Price Range** drawing tool for the [Lightweight Charts v5+](https://github.com/tradingview/lightweight-charts) plugin system. It allows traders to measure the vertical distance between two price levels, automatically displaying the price difference and percentage change. It is part of a modular, high-performance **drop-in replacement** suite for the legacy [v3.8 Line Tools Build](https://github.com/difurious/lightweight-charts-line-tools).
...
1.  **Clone the repository**:
    ```bash
    git clone https://github.com/difurious/lightweight-charts-line-tools-price-range.git
    cd lightweight-charts-line-tools-price-range
    ```
...
4.  **Output**:
    ...
    *   `/docs`: Technical API documentation. Open `docs/index.html` and navigate to the `LineToolPriceRange` class to view all available configuration options (rectangle styling, center line visibility, price label formatting) and methods.
...
Here is how to register and use the Price Range tool in your chart.
...
// 2. Register the Price Range Tool
lineTools.registerLineTool('PriceRange', LineToolPriceRange);

// 3. Start interactive drawing mode (Click Start Point, then move and Click End Point)
lineTools.addLineTool('PriceRange');
```
