# Lightweight Charts Fibonacci Retracement Tool
...
This package provides a specialized **Fibonacci Retracement** drawing tool for the [Lightweight Charts v5+](https://github.com/tradingview/lightweight-charts) plugin system. It allows traders to identify potential support and resistance levels by calculating horizontal lines based on Fibonacci ratios. It is part of a modular, high-performance **drop-in replacement** suite for the legacy [v3.8 Line Tools Build](https://github.com/difurious/lightweight-charts-line-tools).
...
1.  **Clone the repository**:
    ```bash
    git clone https://github.com/difurious/lightweight-charts-line-tools-fib-retracement.git
    cd lightweight-charts-line-tools-fib-retracement
    ```
...
4.  **Output**:
    ...
    *   `/docs`: Technical API documentation. Open `docs/index.html` and navigate to the `LineToolFibRetracement` class to view all available configuration options (level coefficients, colors, opacities) and methods.
...
Here is how to register and use the Fibonacci Retracement tool in your chart.
...
// 2. Register the Fibonacci Retracement Tool
lineTools.registerLineTool('FibRetracement', LineToolFibRetracement);

// 3. Start interactive drawing mode (Click Start/High, then Drag/Click End/Low)
lineTools.addLineTool('FibRetracement');
```
