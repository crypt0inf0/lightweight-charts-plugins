# Lightweight Charts Parallel Channel Tool
...
This package provides a specialized **Parallel Channel** drawing tool for the [Lightweight Charts v5+](https://github.com/tradingview/lightweight-charts) plugin system. It allows traders to define trend channels by drawing a base trend line and an accompanying parallel offset line. It is part of a modular, high-performance **drop-in replacement** suite for the legacy [v3.8 Line Tools Build](https://github.com/difurious/lightweight-charts-line-tools).
...
1.  **Clone the repository**:
    ```bash
    git clone https://github.com/difurious/lightweight-charts-line-tools-parallel-channel.git
    cd lightweight-charts-line-tools-parallel-channel
    ```
...
4.  **Output**:
    ...
    *   `/docs`: Technical API documentation. Open `docs/index.html` and navigate to the `LineToolParallelChannel` class to view all available configuration options (channel colors, middle line visibility, extensions) and methods.
...
Here is how to register and use the Parallel Channel tool in your chart.
...
// 2. Register the Parallel Channel Tool
lineTools.registerLineTool('ParallelChannel', LineToolParallelChannel);

// 3. Start interactive drawing mode 
// (Click P0, then move and Click P1 to set the base line. Finally, move and Click P2 to set the channel width.)
lineTools.addLineTool('ParallelChannel');
```
