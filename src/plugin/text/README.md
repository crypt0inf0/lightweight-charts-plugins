...
This package provides a specialized **Text** annotation tool for the [Lightweight Charts v5+](https://github.com/tradingview/lightweight-charts) plugin system. It allows traders to place descriptive labels and notes directly on the chart at specific coordinates. It is part of a modular, high-performance **drop-in replacement** suite for the legacy [v3.8 Line Tools Build](https://github.com/difurious/lightweight-charts-line-tools).
...
1.  **Clone the repository**:
    ```bash
    git clone https://github.com/difurious/lightweight-charts-line-tools-text.git
    cd lightweight-charts-line-tools-text
    ```
...
4.  **Output**:
    ...
    *   `/docs`: Technical API documentation. Open `docs/index.html` and navigate to the `LineToolText` class to view all available configuration options (font styling, box alignment, scale, rotation) and methods.
...
Here is how to register and use the Text tool in your chart.
...
// 2. Register the Text Tool
lineTools.registerLineTool('Text', LineToolText);

// 3. Start interactive drawing mode (Click to place the text anchor point)
lineTools.addLineTool('Text');
```
