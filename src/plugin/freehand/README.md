# Lightweight Charts Freehand Line Tools (Brush & Highlighter)
...
This package provides specialized **Freehand** drawing tools—the **Brush** and the **Highlighter**—for the [Lightweight Charts v5+](https://github.com/tradingview/lightweight-charts) plugin system. These tools allow for continuous, fluid drawing by clicking and dragging across the chart. It is part of a modular, high-performance **drop-in replacement** suite for the legacy [v3.8 Line Tools Build](https://github.com/difurious/lightweight-charts-line-tools).
...
1.  **Clone the repository**:
    ```bash
    git clone https://github.com/difurious/lightweight-charts-line-tools-freehand.git
    cd lightweight-charts-line-tools-freehand
    ```
...
4.  **Output**:
    ...
    *   `/docs`: Technical API documentation. Open `docs/index.html` and navigate to the `LineToolBrush` or `LineToolHighlighter` classes to view all available configuration options (line width, translucency, colors) and methods.
...
Here is how to register and use the freehand tools in your chart.
...
// 2. Register the Freehand Tools
lineTools.registerLineTool('Brush', LineToolBrush);
lineTools.registerLineTool('Highlighter', LineToolHighlighter);

// 3. Start interactive drawing mode (Click and Hold to draw)
lineTools.addLineTool('Brush');
// or
// lineTools.addLineTool('Highlighter');
```
