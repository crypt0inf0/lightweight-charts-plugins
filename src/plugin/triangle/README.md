# Lightweight Charts Triangle Tool
...
This package provides a specialized **Triangle** drawing tool for the [Lightweight Charts v5+](https://github.com/tradingview/lightweight-charts) plugin system. It allows traders to draw closed geometric shapes defined by exactly three points, perfect for highlighting chart patterns like wedges or pennants. It is part of a modular, high-performance **drop-in replacement** suite for the legacy [v3.8 Line Tools Build](https://github.com/difurious/lightweight-charts-line-tools).
...
1.  **Clone the repository**:
    ```bash
    git clone https://github.com/difurious/lightweight-charts-line-tools-triangle.git
    cd lightweight-charts-line-tools-triangle
    ```
...
4.  **Output**:
    ...
    *   `/docs`: Technical API documentation. Open `docs/index.html` and navigate to the `LineToolTriangle` class to view all available configuration options (background fill, border styles) and methods.
...
Here is how to register and use the Triangle tool in your chart.
...
// 2. Register the Triangle Tool
lineTools.registerLineTool('Triangle', LineToolTriangle);

// 3. Start interactive drawing mode 
// (Click P0, Click P1, then move and Click P2 to finalize the triangle.)
lineTools.addLineTool('Triangle');
```
