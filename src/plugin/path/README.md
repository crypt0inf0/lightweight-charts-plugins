# Lightweight Charts Path (Polyline) Tool
...
This package provides a specialized **Path** drawing tool for the [Lightweight Charts v5+](https://github.com/tradingview/lightweight-charts) plugin system. It allows traders to draw unlimited points for a line defined by a sequence of discrete clicks, making it ideal for mapping complex wave patterns or multi-segment trends. It is part of a modular, high-performance **drop-in replacement** suite for the legacy [v3.8 Line Tools Build](https://github.com/difurious/lightweight-charts-line-tools).
...
1.  **Clone the repository**:
    ```bash
    git clone https://github.com/difurious/lightweight-charts-line-tools-path.git
    cd lightweight-charts-line-tools-path
    ```
...
4.  **Output**:
    ...
    *   `/docs`: Technical API documentation. Open `docs/index.html` and navigate to the `LineToolPath` class to view all available configuration options (line styles, arrow ends) and methods.
...
Here is how to register and use the Path tool in your chart.
...
// 2. Register the Path Tool
lineTools.registerLineTool('Path', LineToolPath);

// 3. Start interactive drawing mode 
// (Click to place P0, Click for subsequent points. Double-Click to finalize the path.)
lineTools.addLineTool('Path');
```
